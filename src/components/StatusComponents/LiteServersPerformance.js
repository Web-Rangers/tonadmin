...
const LiteServersPerformance = ({socketState, data}) => {
    ...
    function updateChart(time_period, time_value){
        const url = `${process.env.REACT_APP_SERVER_URL}/api/v1/chart/server/server_chart?server="${currentServer.current.ip}"&port=${currentServer.current.port}&time_period=${time_period}&time_value=${time_value}`;
